...
# 将224*224的图像分割为14个16*16像素的patch
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape  # [Batch_size, num_patches+1, total_embed_dim] == [8, 197, 768]
        # qkv -> [Batch_size, num_patches+1, 3 * total_embed_dim] == [8, 197, 3*768]
        # reshape -> [Batch_size, num_patches+1, 3, num_heads, embed_dim_per_head] == [8, 197, 3, 8, 96]
        # permute -> [3, Batch_size, num_heads, num_patches+1, embed_dim_per_head] == [3, 8, 8, 197, 96]
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
        ...
        self.patch_embed = embed_layer(img_size=img_size, patch_size=patch_size, in_channel=in_c, embed_dim=embed_dim)
    ...
    def forward_features(self, x):
        # [B, C, H, W] -> [B, num_patches, embed_dim] = [Batch_size, 196, 768]
        # 196为图片裁剪之后拼成的序列长度16*16, 768为维度
        x = self.patch_embed(x)  # [B, 196, 768]
        # 生成Transformer需要的start token, shape与x相同, 长度为1
        # [1, 1, 768] -> [B, 1, 768]
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)
        if self.dist_token is None:
            # 将start token与x拼接，形成一个长度为197的序列
